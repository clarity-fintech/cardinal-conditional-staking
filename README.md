# Cardinal Conditional Staking

> **Moniversive Invariant Static (MIS)** module · settles on **CLRTY-1** (chain **1202**) · compiler **`misc`**  
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/cardinal-conditional-staking`](https://github.com/clarity-fintech/cardinal-conditional-staking)
...
**Cardinal Conditional Staking** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/cardinal-conditional-staking
cd cardinal-conditional-staking
...
| `bin/misc mis/kernel/CardinalConditionalStakingKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/CardinalConditionalStakingKernel.mis --check --compact-letters
...
```
mis/kernel/CardinalConditionalStakingKernel.mis
mis/packs/condition_seal.mis
mis/packs/conditional_stake.mis
mis/packs/expiry_release.mis
mis/packs/nft_rental.mis
mis/packs/temp_delegation.mis
...
mis/commands/CardinalConditionalStakingCommandCatalog.mis
mis/commands/CardinalConditionalStakingCommands.mis
mis/commands/CardinalConditionalStakingNetworkBind.mis
...
`mis/kernel/CardinalConditionalStakingKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: Cardinal Conditional Staking Kernel

module CardinalConditionalStakingKernel {
  ...
  invariant domain_bound: domain == cardinal_conditional;
  ...
  invariant pack_conditional_stake_flag: pack_conditional_stake_bound == true;
  invariant pack_nft_rental_flag: pack_nft_rental_bound == true;
  invariant pack_temp_delegation_flag: pack_temp_delegation_bound == true;
  invariant pack_condition_seal_flag: pack_condition_seal_bound == true;
  invariant pack_expiry_release_flag: pack_expiry_release_bound == true;
...
// Creator: Chandler William Ferguson
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Cardinal Conditional Staking · ConditionSeal

module CardinalConditionalStakingConditionSeal {
  invariant letter_hash_bound: letter_hash_root != @0;
  invariant deep_root_moniversive: deep_root == moniversive;
  invariant settlement_chain: chain_id == 1202;
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_condition_seal_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant condition_seal_061: condition_seal_061_bound == true;
  invariant condition_seal_062: condition_seal_062_bound == true;
  invariant condition_seal_063: condition_seal_063_bound == true;
  invariant condition_seal_064: condition_seal_064_bound == true;
  invariant condition_seal_065: condition_seal_065_bound == true;
  invariant condition_seal_066: condition_seal_066_bound == true;
  invariant condition_seal_067: condition_seal_067_bound == true;
  invariant condition_seal_068: condition_seal_068_bound == true;
```
...
```
CONNECTED clrty-1/1202 cardinal-conditional-staking
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/CardinalConditionalStakingCommands.mis
```
...
{
  "ok": true,
  "module": "CardinalConditionalStakingBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1179,
...
*Reproduce:* `make check` → `"ok": true`, module `CardinalConditionalStakingBacklinkIndex`, 9 invariants, 4 outcomes, 1179 typed letters.
...
- [clarity-fintech/cardinal-conditional-staking](https://github.com/clarity-fintech/cardinal-conditional-staking) — this repository
