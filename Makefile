...
check:
	...
	$(MISC) mis/kernel/CardinalConditionalStakingKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK cardinal-conditional-staking
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/CardinalConditionalStakingCommands.mis
